...
//            s_airBubbleRatio = 0.3;
            ...
            if (METER_DIR_DRAIN == s_dir)    //向下排空定量常用于小体系模式
            {
                ...
                s_limitVolume = 0;
                MeterLED_TurnOn(s_setMeterPoint);
...
static void Meter_Stop(MeterResult result)
{
    ...
    {
        MeterLED_TurnOff(s_setMeterPoint);
    }
    else
    {
...
/**
 * @brief  打开定量使用的LED灯，仅供上层命令控制LED
 * @param num 定量点1到定量点METER_POINT_NUM
 * @return TRUE 操作成功，FALSE操作失败
 */
Bool Meter_TurnOnLED(Uint8 num)
{
    return MeterLED_TurnOn(num-1);
}

/**
 * @brief  关闭定量使用的LED灯，仅供上层命令控制LED
 * @param Num 定量点1到定量点METER_POINT_NUM
 * @return TRUE 操作成功，FALSE操作失败
 */
Bool Meter_TurnOffLED(Uint8 num)
{
    return MeterLED_TurnOff(num-1);
}

/**
 * @brief  获取LED光强等级，仅供上层命令控制LED
 * @param num 定量点1到定量点METER_POINT_NUM
 * @return 光强等级
 */
Uint16 Meter_GetLEDLevel(Uint8 num)
{
    return MeterLED_GetCurrentLevel(num-1);
}

/**
 * @brief 设置LED光强等级，仅供上层命令控制LED
 * @param num 定量点1到定量点METER_POINT_NUM
 * @param 光强等级
 * @return TRUE 操作成功，FALSE操作失败
 */
Bool Meter_SetLEDLevel(Uint8 num, Uint16 value)
{
    if(TRUE == MeterLED_SetCurrentLevel(num-1, value))
    {
        MeterLED_SetDefaultLevel(num-1, value);
        return TRUE;
    }
    else
    {
