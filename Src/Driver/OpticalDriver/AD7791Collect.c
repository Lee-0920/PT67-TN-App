...
// CS_MUTEX
#define AD7791_MUTEX_PIN           GPIO_Pin_12
#define AD7791_MUTEX_GPIO          GPIOB
#define AD7791_MUTEX_RCC           RCC_AHB1Periph_GPIOB
#define AD7791_MUTEX_HIGH()        GPIO_SetBits(AD7791_MUTEX_GPIO, AD7791_MUTEX_PIN)
#define AD7791_MUTEX_LOW()         GPIO_ResetBits(AD7791_MUTEX_GPIO, AD7791_MUTEX_PIN)

// CS
#define AD7791_CS_PIN           GPIO_Pin_2
...
//#define IGL_CTRL_PIN            GPIO_Pin_8
...
//
//#define IGL_RESET_PIN           GPIO_Pin_9
 ...
 * @param
 */
static uint8_t AD7791Collect_AD7791SendByte(uint8_t senddata)
{
 ...
 * @param
 */
static Bool AD7791Collect_AD7791Ready(uint32_t timeoutms)
{
    uint32_t loopms = 0;
 ...
 * @param
 */
static uint32_t AD7791Collect_AD7791Read24bitData()
{
    uint32_t read_data = 0;
    // 读数据
    read_data = 0;
    read_data |= AD7791Collect_AD7791SendByte(AD7791_DUMMY_BYTE) << 16;
    read_data |= AD7791Collect_AD7791SendByte(AD7791_DUMMY_BYTE) << 8;
    read_data |= AD7791Collect_AD7791SendByte(AD7791_DUMMY_BYTE);
    return read_data;
}
...
void AD7791Collect_Init(void)
{
    ...
    // CS_MUTEX
    GPIO_InitStructure.GPIO_Pin = AD7791_MUTEX_PIN;
    GPIO_Init(AD7791_MUTEX_GPIO, &GPIO_InitStructure);
    ...
    // 复位
    AD7791_CS_LOW();    // 使能
    AD7791Collect_AD7791SendByte(0xff);       // 复位芯片，最少32个时钟的 DIN 高电平
    AD7791Collect_AD7791SendByte(0xff);
    AD7791Collect_AD7791SendByte(0xff);
    AD7791Collect_AD7791SendByte(0xff);
    AD7791Collect_AD7791SendByte(0xff);
    AD7791Collect_AD7791SendByte(0xff);
    ...
    // 设置采样频率
    AD7791_CS_LOW();    // 使能
    AD7791Collect_AD7791SendByte(AD7791_CMD_SETTING_FILTER);
    AD7791Collect_AD7791SendByte(AD7791_UPDATE_RATES_33_3 & 0x07);
...
uint32_t AD7791Collect_GetAD(Uint8 channel)
{
    ...
    OpticalChannel_Select(AD7791, channel);
    System_Delay(20);
    AD7791_CS_LOW();    // 使能
    AD7791Collect_AD7791SendByte(AD7791_CMD_SETTING_MODE);
    AD7791Collect_AD7791SendByte(
            AD7791_MODE_CONV_SINGLE | AD7791_MODE_BUFFER | s_AD7791PolarMode);
    if (0 == AD7791Collect_AD7791Ready(AD7791_COMMUNICATION_TIMEOUT_MS)) // 等待转换完成
    {
        TRACE_ERROR("\n AD7791 collect time over");
        return 0;   // 如果超时
    }
    // 读取数据
    AD7791Collect_AD7791SendByte(AD7791_CMD_READ_SINGLE);
    data = AD7791Collect_AD7791Read24bitData();
    AD7791_CS_HIGH();   // 失能
