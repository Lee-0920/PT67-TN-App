...
static int Cmd_ADTest(int argc, char *argv[]);
...
const CmdLineEntry g_kConsoleCmdTable[] =
{
    ...
    { "adtest",     Cmd_ADTest,      "\t\t: Optical Channel Test." },
...
int Cmd_MeterLED(int argc, char *argv[])
{
    ...
    else if (IsEqual(argv[1], "level"))
    {
        if (IsEqual(argv[2],"get") && argv[3])
        {
            Printf("\n Meter LED  level is %d",Meter_GetLEDLevel(atoi(argv[3])));
        }
        else if(IsEqual(argv[2],"set") && argv[3] && argv[4])
        {
            if(TRUE == Meter_SetLEDLevel(atoi(argv[3]), atoi(argv[4])))
            {
                Printf("\n Set Meter LED %d  level %d success. ", atoi(argv[3]), atoi(argv[4]));
            }
        }
        else
        {
    ...
    else if (0 == argv[1] || IsEqual(argv[1], "help") || IsEqual(argv[1], "?"))
    {
        ...
        Printf(" meterled level get [NUM]: Close NUM LED,NUM: 1 - %d.\n", METER_POINT_NUM);
        Printf(" meterled level set [NUM][LEVEL]: Close NUM LED,NUM: 1 - %d.\n", METER_POINT_NUM);
    }
    else
    {
        Printf("Invalid param: %s\n", argv[1]);
...
//
//*****************************************************************************
int Cmd_ADTest(int argc, char *argv[])
{
    if (IsEqual(argv[1], "open"))
    {
        if (argv[2])
        {
            ...
            {
                OpticalControl_CollectTestFun(TRUE, atoi(argv[2]));
            }
            else
            {
                Printf("\n Channel 1 - 2");
    ...
    {
        OpticalControl_CollectTestFun(FALSE, 1);
        Printf("\n Close collect test fun.");
    }
    else if (0 == argv[1] || IsEqual(argv[1], "help") ||
             IsEqual(argv[1], "?"))
    {
        Printf("====== adtest commands ======\n");
        Printf(" adtest open [CHANNEL] :Single channel signal acquisition and open,  test channel \n");
        Printf(" adtest close      :Single channel signal acquisition and close.\n");
        Printf(" ref channel %d\n", PD_REF_CHANNEL);
        Printf(" mea channel %d\n", PD_MEA_CHANNEL);
    }
    else
    {
        Printf("Invalid param: %s\n", argv[1]);
...
int Cmd_OAI(int argc, char *argv[])
{
    ...
    else if (IsEqual(argv[1], "start"))
    {
        ...
        {
            if(argv[3] && atof(argv[3]) >= 0)
            {
                OpticalControl_SendEventClose();
                OpticalControl_StartAcquirer((atoi(argv[2])), atof(argv[3]));
            }
            else
            {
                Printf("\n Invalid param");
    ...
    else if (IsEqual(argv[1], "mode"))
    {
        if (IsEqual(argv[2], "get"))
        {
            Printf("\n optical control mode is %d", (Uint8)OpticalControl_GetWorkMode());
        }
        else if(IsEqual(argv[2], "set"))
        {
            if(argv[3] && atoi(argv[3]) >= 0)
            {
                OpticalControl_ResetWorkMode(atoi(argv[3]));
            }
            else
            {
                Printf("\n Invalid param");
    ...
    {
        Printf("====== oai commands ======\n");
        Printf(" oai getad  start [TIME]:Ad value of the optical is continuously display.Display interval [TIME]\n");
        Printf(" oai getad  stop         :Stop continue to collect the ad value of the current channel.\n");
        Printf(" oai start  [INDEX][TIME]      :Start collect ad value [INDEX 0-AD7791 1-ADS1146][TIME] s.\n");
        Printf(" oai stop                : \n");
        System_Delay(5);
        Printf(" oai report [TIME]      :Sets the current ad value reporting period. Uint is s.\n");
        Printf(" oai mode get      :get optical module work mode.\n");
        Printf(" oai mode set  [TYPE]    :set optical module work mode 0-AD7791 1-ADS1146 2-BOTH.\n");
        System_Delay(5);
    }
    else
    {
...
int Cmd_StaticAD(int argc, char *argv[])
{
             ...
             IsEqual(argv[1], "?"))
    {
        ...
        Printf(" staticad start  [index] [target] : start static ad control. [index: tpref=0,tpmea=1,meter1=2,meter2=3,tn220=4,tn275=5]\n");
...
int Cmd_MeasureLED(int argc, char *argv[])
{
    ...
    else if (IsEqual(argv[1], "target"))
    {
        if (IsEqual(argv[2], "get"))
        {
            Printf("\n LED controller target: %d", LEDController_GetTarget());
        }
        else if (IsEqual(argv[2], "set"))
        {
            if (argv[3] && atoi(argv[3]) > 0)
            {
                LEDController_SetTarget(atoi(argv[3]));
            }
            else
            {
                Printf("Invalid param: %s\n", argv[2]);
    ...
    else if (IsEqual(argv[1], "pid"))
    {
       ...
       if (IsEqual(argv[2], "get"))
       {
           ...
           Printf("\n p =");
           System_PrintfFloat(1, ledControllerParam.proportion, 5);
           Printf("\n i =");
           System_PrintfFloat(1, ledControllerParam.integration, 5);
           Printf("\n d =");
           System_PrintfFloat(1, ledControllerParam.differential, 5);
       }
       else if (IsEqual(argv[2], "set"))
       {
           if (argv[3] && argv[4] && argv[5])
           {
               ledControllerParam.proportion = atof(argv[3]);
               ledControllerParam.integration = atof(argv[4]);
               ledControllerParam.differential = atof(argv[5]);
    ...
    else if (IsEqual(argv[1], "dacdef"))
    {
        if (IsEqual(argv[2], "get"))
        {
            ...
            System_PrintfFloat(1, OpticalLed_GetDefaultValue(), 3);
            Printf(" V");
        }
        else if (IsEqual(argv[2], "set"))
        {
             ...
             IsEqual(argv[1], "?"))
    {
        ...
        Printf(" mealed on                 : Just open LED.\n");
        Printf(" mealed start              : Turn on the LED light and start the PID controller.\n");
        Printf(" mealed close              : Turn off the LED light and close PID controller. \n");
        System_Delay(3);
        Printf(" mealed target get         : Get PID target value.\n");
        Printf(" mealed target set [AD]    : Set PID target value.\n");
        System_Delay(3);
        Printf(" mealed pid get            : Get PID parameters. \n");
        Printf(" mealed pid set [P] [I] [D]: Set PID parameters. \n");
        System_Delay(3);
        Printf(" mealed dacdef get   : Get the LED DAC default voltage. \n");
        Printf(" mealed dacdef set   : Set the LED DAC default voltage. \n");
        Printf(" mealed dacout      : Change the control voltage of LED. \n");
        System_Delay(3);
    }
    else
    {
...
int Cmd_ChangePolar(int argc, char *argv[])
{
    ...
    {
        AD7791Collect_ChangePolar(AD7791_MODE_UNIPOLAR);
    }
    else if (IsEqual(argv[1], "bipolay"))
    {
        AD7791Collect_ChangePolar(AD7791_MODE_BIPOLAR);
    }
    else if (0 == argv[1] || IsEqual(argv[1], "help") ||
             IsEqual(argv[1], "?"))
    {
...
int Cmd_Relay(int argc, char *argv[])
{
    if (IsEqual(argv[1], "on"))
     {
        if (IsEqual(argv[2], "all"))
         {
            Relay_TurnOnAll();
         }
        else if((atoi(argv[2]) >= 0) && (atoi(argv[2]) <= Relay_TotalNumber()))
        {
            Relay_TurnOn(atoi(argv[2]));
        }
        else
        {
            Printf("\n Invalid Relay on param");
        }
     }
     else if (IsEqual(argv[1], "off"))
     {
         ...
         {
             Relay_TurnOff(atoi(argv[2]));
         }
         else
         {
             Printf("\n Invalid Relay off param");
         }
     }
     else if (IsEqual(argv[1], "status"))
     {
         if((atoi(argv[2]) >= 0) && (atoi(argv[2]) <= Relay_TotalNumber()))
          {
              Relay_IsOpen(atoi(argv[2]));
          }
          else
          {
              Printf("\n Invalid Relay status param");
          }
     }
     else if (IsEqual(argv[1], "num"))
     {
         Printf("\n Relay Number %d", Relay_TotalNumber());
     }
     else if (0 == argv[1] || IsEqual(argv[1], "help") ||
              IsEqual(argv[1], "?"))
     {
         Printf("====== relay commands ======\n");
         Printf(" relay on [num]: 0 ~ NUM-1 or all\n");
         Printf(" relay off [num]: 0 ~ NUM-1 or all\n");
         Printf(" relay status [num]: 0 ~ NUM-1\n");
         Printf(" relay num: get relay total number NUM\n");
     }
     else
     {
         Printf("Invalid param: %s\n", argv[1]);
