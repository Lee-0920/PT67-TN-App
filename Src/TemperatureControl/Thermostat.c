...
{
    Uint8 readData[THERMOSTAT_PARAM_LEN] =
    { 0 };
...
static Bool Thermostat_TempCheck(Thermostat * thermostat)
{
    ...
    {
        if((thermostat->targetTemp - curTemp) > 10)
        {
            thermostat->checkTemp[thermostat->checkIndex++] = curTemp;

            if(thermostat->checkIndex >= 20)
            {
                thermostat->checkIndex= 0;
                deviation = CalculateMaxDeviation(thermostat->checkTemp, 20);
                if(deviation < 0.5)
                {
                    thermostat->errTimes++;
