...
static void ThermostatManager_TemperatureReport(void *argument)
{
    Uint8 data[4 + 4 * TOTAL_THERMOSTAT] = { 0 };
    ...
    while(1)
    {
        ...
        {
            temp = Thermostat_GetCurrentTemp(&s_thermostats[0]);  //兼容63平台温度上报顺序
            ...
            for (Uint8 i = 1; i < TOTAL_THERMOSTAT; i++)
            {
                temp = Thermostat_GetCurrentTemp(&s_thermostats[i]);
                memcpy(data + 4 + i * 4, &temp, sizeof(temp));
            }
