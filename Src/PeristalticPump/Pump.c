...
{
    Uint8 buffer[PUMP_SIGN_FLASH_LEN] =
    { 0 };
    Uint32 flashFactorySign = 0;

    pump->status = PUMP_IDLE;

    McuFlash_Read(PUMP_SIGN_FLASH_BASE_ADDR + PUMP_SIGN_FLASH_LEN * pump->number,
    ...
    else
    {
        ...
        McuFlash_Write(
                PUMP_SIGN_FLASH_BASE_ADDR + PUMP_SIGN_FLASH_LEN * pump->number,
...
{
    if ( FALSE
            == PumpTimer_RegisterHandle((TimerHandle) Pump_MoveHandler, pump))
    {
        return FALSE;
    }
